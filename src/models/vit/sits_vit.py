...
Diferencias clave vs un ViT estándar de imágenes:
    ...
    - La salida es un logit de clasificación binaria (sano / estresado).
...
Arquitectura:
    ...
    └────────────────┬────────────────────────────────────────────┘
                     │
    ┌────────────────▼────────────────────────────────────────────┐
    │  ClassificationHead  Linear(d_model → 1)  logit de estrés   │
    └─────────────────────────────────────────────────────────────┘
"""
...
class TemporalPositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, max_doy: int = 366):
        ...
        # Tabla precalculada: (max_doy+1, d_model)
        ...
        # Buffer (no parámetro entrenable) que se mueve a GPU automáticamente
    ...
    def forward(self, doy: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Clamp por seguridad ante valores fuera de rango
...
class SITSTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn  = nn.MultiheadAttention(
            ...
            num_heads   = num_heads,
            dropout     = dropout,
            batch_first = True,   # (B, T, d_model) como convención
        )
...
class MaskedTemporalPooling(nn.Module):
    ...
    def forward(
        ...
        # Invertir: True donde hay dato real
...
class SITSViT(nn.Module):
    ...
    max_seq_len: longitud máxima de secuencia T para validación (default 200)
    ...
    Salida:
        logit : (B, 1)  — logit de estrés hídrico (sin sigmoid)

    Uso típico:
        model  = SITSViT(n_channels=5, d_model=128, num_heads=8, num_layers=4)
        logit  = model(x, doy)                 # (B, 1)
        prob   = torch.sigmoid(logit)          # probabilidad de estrés
        stress = (prob >= 0.5).float()         # predicción binaria
    """
    ...
    def __init__(
        ...
        # Lleva cada vector espectral (C,) al espacio del modelo (d_model,)
    ...
    def forward(
        ...
        retorna      : (B, 1)     logit crudo de estrés
        """
    ...
    def name(self) -> str:
        return (
            f"SITSViT(d={self.d_model}, h={self.num_heads}, L={self.num_layers})"
        )
...
def sits_collate_fn(
    ...
    Cuando distintas parcelas tienen diferente número de fechas T, PyTorch
    no puede apilarlas directamente. Esta función:
        1. Encuentra el T máximo del batch
        2. Pad con ceros las series más cortas
        3. Genera la padding_mask (True donde hay ceros de relleno)

    Cada elemento del batch debe ser una tupla:
        (x: Tensor (T, C),  doy: Tensor (T,),  label: Tensor (1,))
    ...
    labels = torch.stack(labels, dim=0)   # (B, 1)
...
def build_vit(
    ...
    **kwargs,
    ...
    Configuraciones de referencia (equilibrio capacidad/velocidad):
        Tiny  : d_model=64,  num_heads=4, num_layers=2  (~  80K params)
        Small : d_model=128, num_heads=8, num_layers=4  (~ 330K params) ← default
        Base  : d_model=256, num_heads=8, num_layers=6  (~1.3M params)
    """
    return SITSViT(
