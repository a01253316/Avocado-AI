...
DEFAULT_CONFIG = {
    ...
    "ndmi_threshold":   -0.1,
    "window_size":      24,    # fechas por ventana temporal (~3 meses)
    "window_step":      4,     # paso entre ventanas (~20 días)
    "stress_frac_min":  0.25,  # fracción mínima de fechas estresadas → label=1
    "exclude_ndmi":     False, # si True, NDMI se excluye del input (baseline honesto)
    # Modelo
...
def train(cfg: dict) -> None:
    ...
    try:
        ...
        # Canales excluidos del input (NDMI siempre se usa para el label)
        exclude_channels = ["NDMI"] if cfg.get("exclude_ndmi") else []
        n_channels_input = cfg["n_channels"] - len(exclude_channels)

        if exclude_channels:
            logger.info(
                "⚠️  Baseline honesto: NDMI excluido del input  "
                "(label sigue siendo NDMI < threshold)  |  n_channels=%d",
                n_channels_input,
            )
        ...
        datasets = load_datasets(
            ...
            window_step     = cfg.get("window_step", 4),
            stress_frac_min = cfg.get("stress_frac_min", 0.25),
        ...
        if "test" in loaders:
            ...
            if cm.shape == (2, 2):
                ...
                present = np.unique(
                    np.concatenate(
                        [t.detach().cpu().numpy().flatten() for _, t in loaders["test"]]
                    ).astype(int)
                )
                logger.warning(
                    "  ⚠️  Solo clase(s) %s en test (cm=%s). "
                    "TN/FP/FN/TP no aplica — el modelo predice una sola clase. "
                    "Revisa el umbral NDMI o agrega más parcelas.",
                    present.tolist(), cm.shape,
                )
...
def main() -> None:
    ...
    parser.add_argument("--window-size", type=int, default=24,
                        help="Fechas por ventana temporal (default: 24 ≈ 3 meses)")
    parser.add_argument("--window-step", type=int, default=4,
                        help="Paso entre ventanas en fechas (default: 4 ≈ 20 días)")
    parser.add_argument("--stress-frac", type=float, default=0.25,
                        help="Fracción mínima de fechas estresadas en ventana → label=1 (default: 0.25)")
    parser.add_argument("--exclude-ndmi", action="store_true",
                        help=(
                            "Excluye NDMI del input del modelo. "
                            "El label sigue siendo NDMI < threshold. "
                            "Úsalo para un baseline honesto sin data leakage."
                        ))
    ...
    cfg = {
        ...
        "stress_frac_min":  args.stress_frac,
        "exclude_ndmi":     args.exclude_ndmi,
        "n_channels":       5,
